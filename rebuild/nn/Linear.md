...
${\hat{\mathbf{y}}} = \mathbf{x} \mathbf{w} + \mathbf{b}$, 这里$\mathbf{x}$ 的shape是`[batch, d_input]`, $\mathbf{w}$ 的shape是`[d_input, d_output]`, $\mathbf{b}是`[batch, d_output]`. 而且其一可以退化为向量。

`X`应该这样排列, **每一行是一个样本, 每一列是一种特征**.
