...
- `parameters()` 和 `named_parameters()` 返回的是一个 各层 `torch.nn.parameter.Parameter` 生成器generator，其包含权重`.data`、梯度`.grad`、设备`.device`三个信息
...
`<class 'torch.nn.parameter.Parameter'>`包含:
- 值: `.data`
- 梯度: `.grad`
